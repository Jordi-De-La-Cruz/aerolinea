const cart = {
    ...
    save() {
        try { localStorage.setItem("anioCart", JSON.stringify(this.items)); } catch { }
    },

    load() {
        try {
            const saved = localStorage.getItem("anioCart");
...
function initCartToggle() {
    ...
    const open = () => { dropdown.hidden = false; toggleBtn?.setAttribute("aria-expanded", "true"); };
    const close = () => { dropdown.hidden = true; toggleBtn?.setAttribute("aria-expanded", "false"); };
